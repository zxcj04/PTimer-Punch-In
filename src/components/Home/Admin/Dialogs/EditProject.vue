    ...
    <v-card>
      ...
      <v-card-text>
        <v-form ref="form" @submit.prevent validate-on="lazy">
          <v-row>
            <v-col cols="12" md="6">
              <v-text-field label="專案名稱" v-model="editedItem.project_name" :rules="[rules.required]" hint="必填"
            ...
            <v-col cols="12">
              <v-text-field label="專案描述" v-model="editedItem.description" :rules="[rules.required]" hint="必填"
                persistent-hint></v-text-field>
            </v-col>
          </v-row>
        </v-form>
...
const props = defineProps(['dialog', 'project']);
const emit = defineEmits(['update:dialog', 'update:project', 'editItem']);
...
const onOpen = () => {
  ...
  _selectedUserName.value = props.project.project_owner_name;
};
...
const save = async () => {
  ...
  emit('editItem', props.project.project_id, editedItem.value);
...
const selectedUserName = computed({
  ...
  set(value) {
    ...
    editedItem.value.project_owner = toUserId(value);
  }
});
